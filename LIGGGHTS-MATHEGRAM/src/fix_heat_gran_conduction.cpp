...
FixHeatGranCond::~FixHeatGranCond()
{
  ...
  if (conductivity_ && !variable_conductivity_flag_)
...
void FixHeatGranCond::init()
{
  ...
  // check if conductivity is temperature sensitive
  fix_variable_thermal_conductivity_ =
     static_cast<FixPropertyAtom*>(modify->find_fix_property("variableThermalConductivity","property/atom","scalar",0,0,style,false));
  if (fix_variable_thermal_conductivity_) variable_conductivity_flag_ = 1; 
...
void FixHeatGranCond::post_force_eval(int vflag,int cpl_flag)
{
  ...
  if(variable_conductivity_flag_) {
     conductivity_ = fix_variable_thermal_conductivity_->vector_atom; 
  }
