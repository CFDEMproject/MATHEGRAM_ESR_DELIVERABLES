...
-------------------------------------------------------------------------
    ...
    Implementation of temperature-sensitive heat capacity
...
{
  fix_quantity = fix_flux = fix_source = NULL; 
  fix_capacity = NULL; fix_variable_thermal_capacity = NULL; 
...
FixScalarTransportEquation::~FixScalarTransportEquation()
{
    ...
    if(capacity && !variable_capacity_flag) delete []capacity;
...
void FixScalarTransportEquation::init()
{
  ...
  if(capacity_flag)
  {
      // see if capacity is temperature sensitive
      fix_variable_thermal_capacity = 
          static_cast<FixPropertyAtom*>(modify->find_fix_property("variableThermalCapacity","property/atom","scalar",0,0,style,false)); 

      if (fix_variable_thermal_capacity) {
         variable_capacity_flag = 1; 
         capacity = fix_variable_thermal_capacity->vector_atom; 
      }
      else {
...
void FixScalarTransportEquation::advanceQtyExplicit()
{
    ...
    if(capacity_flag)
    {
        ...
        {
           if (mask[i] & groupbit){
              if (variable_capacity_flag) {
                 capacity = fix_variable_thermal_capacity->vector_atom[i]; 
              }
              else {
